...
import { AddHostsContextProvider, AlertsContextProvider, Cluster, CpuArchitecture, ErrorState, InfraEnv, ResourceUIState } from '../../../common';
...
const ClusterPage: React.FC<RouteComponentProps<MatchParams>> = ({ match }) => {
  ...
  const getContent = (cluster: Cluster, infraEnv: InfraEnv) => {
    if (cluster.status === 'adding-hosts') {
      ...
      ['preparing-for-installation', 'installing', 'installing-pending-user-action', 'finalizing', 'installed', 'error', 'cancelled'].includes(
        cluster.status
      )
    ) {
      return (
    ...
    return (
      <PageSection variant={PageSectionVariants.light} isFilled>
        <ErrorState
          ...
          fetchData={Number(errorDetail?.code) === 401 ? () => window.location.reload() : fetchCluster}
    ...
    return (
      <PageSection variant={PageSectionVariants.light} isFilled>
        <ErrorState
          ...
          actions={[<BackButton key={'cancel'} to={`${routeBasePath}/clusters`} />]}
          content={'Check to make sure the cluster-ID is valid. Otherwise, the cluster may have been deleted.'}
        />
      </PageSection>
    );
  }
      ...
      <AlertsContextProvider>
        <SentryErrorMonitorContextProvider>
          <ModalDialogsContextProvider>
            <ClusterDefaultConfigurationProvider loadingUI={<ClusterLoading />} errorUI={<ClusterUiError />}>
